...
import com.wl.weblog.web.model.vo.archive.FindArchiveArticlePageListRspVO;
...
public class ArchiveServiceImpl implements ArchiveService {
    ...
    public Response findArchivePageList(FindArchiveArticlePageListReqVO findArchiveArticlePageListReqVO) {
        ...
        if (!CollectionUtils.isEmpty(articleDOS)) {
            // DO 转 VO
            List<FindArchiveArticleRspVO> archiveArticleRspVOS = articleDOS.stream().map(articleDO -> ArticleConvert.INSTANCE.convertDO2ArchiveArticleVO(articleDO)).collect(Collectors.toList());
