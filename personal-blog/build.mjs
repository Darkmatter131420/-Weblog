...
published.forEach((p,i) => { const parsed=markdown(p.content), prev=published[i+1], next=published[i-1]; const toc=parsed.toc.length?`<aside class="toc"><strong>目录</strong>${parsed.toc.map(x=>`<a href="#${x.id}">${x.text}</a>`).join('')}</aside>`:''; const body=`<main class="article container"><a class="read-more" href="../">← 返回首页</a><p class="eyebrow">${p.tags.map(esc).join(' · ')} · ${p.date} · ${readingTime(p.content)} 分钟阅读</p><h1>${esc(p.title)}</h1><p class="lead">${esc(p.excerpt)}</p><div class="article-layout"><div><div class="article-body">${parsed.html}</div><nav class="post-nav">${prev?`<a href="${prev.slug}.html">← ${esc(prev.title)}</a>`:'<span>'}${next?`<a href="${next.slug}.html">${esc(next.title)} →</a>`:'<span>'}</nav></div>${toc}</div></main>`; fs.writeFileSync(path.join(siteDir,'posts',`${p.slug}.html`), shell(p.title,p.excerpt,body,`https://darkmatter131420.github.io/-Weblog/personal-blog/posts/${p.slug}.html`)); });
...
const publicMeta=published.map(({content,...p})=>({...p,readingTime:readingTime(content)}));
fs.writeFileSync(path.join(siteDir,'posts.json'),JSON.stringify(publicMeta,null,2));
const feed=`<?xml version="1.0" encoding="UTF-8"?><rss version="2.0"><channel><title>Darkmatter · 个人博客</title><link>https://darkmatter131420.github.io/-Weblog/personal-blog/</link><description>Darkmatter 的个人博客</description>${published.map(p=>`<item><title>${esc(p.title)}</title><link>https://darkmatter131420.github.io/-Weblog/personal-blog/posts/${p.slug}.html</link><pubDate>${new Date(p.date).toUTCString()}</pubDate><description>${esc(p.excerpt)}</description></item>`).join('')}</channel></rss>`;
fs.writeFileSync(path.join(siteDir,'feed.xml'),feed);
fs.writeFileSync(path.join(siteDir,'sitemap.xml'),`<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"><url><loc>https://darkmatter131420.github.io/-Weblog/personal-blog/</loc></url><url><loc>https://darkmatter131420.github.io/-Weblog/personal-blog/archive.html</loc></url>${published.map(p=>`<url><loc>https://darkmatter131420.github.io/-Weblog/personal-blog/posts/${p.slug}.html</loc><lastmod>${p.date}</lastmod></url>`).join('')}</urlset>`);
