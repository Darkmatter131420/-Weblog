...
posts.forEach(p => fs.writeFileSync(path.join(siteDir,'posts',`${p.slug}.html`), shell(p.title, `<main class="article container"><a class="read-more" href="../">← 返回首页</a><p class="eyebrow">${p.tags.map(esc).join(' · ')} · ${p.date}</p><h1>${esc(p.title)}</h1><p class="lead">${esc(p.excerpt)}</p><div class="article-body">${markdown(p.content)}</div></main>`)));
